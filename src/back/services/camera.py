    ...
    def _run(self) -> None:
        while self._running:
            ...
            try:
                ...
                logger.debug("Inference: %d detections", len(detections))

                # Live inference is a visual overlay only — no live count. The
                # authoritative number is computed offline by the counting-worker.
