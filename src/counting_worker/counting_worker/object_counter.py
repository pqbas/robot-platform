"""Line-crossing counter — copied verbatim from
``back/services/perception/object_counter.py``.

Workers are isolated uv projects; the backend never imports worker code and
vice versa (repo invariant). This is ~60 lines of pure geometry with no torch
dependency, so we duplicate it rather than share a package. Keep it in sync
...
class ObjectCounter:
    def __init__(self, count_mode: str, threshold: float, direction: str):
        ...
        # the processor divides centroids by frame W/H before calling update(),
        # so the comparison is resolution-independent.
